...
Wizard.prototype = {
	...
	setState : function() {
		...
		$steps.find('span.badge').removeClass('badge-primary').removeClass(
				'badge-success');
		...
		if ($('.wizard .actions').length) {
			containerWidth = $('.wizard').width()
					- $('.wizard .actions').outerWidth();
		} else {
			containerWidth = $('.wizard').width();
		}
		if (totalWidth > containerWidth) {
			var newMargin = totalWidth - containerWidth;
			$('.wizard .steps').attr('style',
	...
	stepclicked : function(e) {
		...
		$("#nextBtnAdd").show();
		$("#preBtnAdd").hide();
		$("#agreeProjectAddBtn").hide();
		$("#refuseProjectAddBtn").hide();
	...
	next : function() {
		...
		$("#preBtnAdd").show();
		//如果是信息办领导拒绝的，没有通过按钮
		if($("#modelLc").val() == 4 && $("#modelState").val() == 3){
			$("#agreeProjectAddBtn").hide();
		}else{
			$("#agreeProjectAddBtn").show();
		}
		$("#refuseProjectAddBtn").show();
